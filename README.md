...
Este comando tiene una sola misión: decirle a APT dónde descargar ROS2. Se ejecuta en tres etapas:

**① Subcomandos dinámicos** — antes de que `echo` haga cualquier cosa, el shell evalúa:
- `$(dpkg --print-architecture)` — detecta si tu máquina es `amd64`, `arm64`, etc.
- `$(. /etc/os-release && echo $UBUNTU_CODENAME)` — lee tu versión de Ubuntu (`jammy`, `focal`, etc.) para usar los paquetes compatibles.

**② `echo`** toma esos valores y construye la línea con el formato exacto que APT espera para un repositorio externo, incluyendo la ruta a la clave GPG que verifica la autenticidad de los paquetes.

**③ `sudo tee`** — el símbolo `|` manda esa línea como entrada a `tee`, que necesita `sudo` porque `/etc/apt/sources.list.d/` pertenece a root. `tee` normalmente también imprime en pantalla, pero `> /dev/null` descarta eso para una salida limpia.

> La próxima vez que ejecutes `apt update`, Ubuntu ya sabe dónde buscar los paquetes de ROS2.

### Flujo del comando

![Diagrama del comando apt de ROS2](ros2_apt_diagram.svg)
